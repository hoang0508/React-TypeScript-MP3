...
export default function PlayMusicAudio({
  ...
  const { durationTime, remainingTime, range, handleChangePlay } =
  ...
  return (
    <div className="mt-4">
      <input
        type="range"
        name="range"
        value={refMp3.current !== Number ? 0 : refMp3.current?.currentTime}
        ref={range}
        className="range"
        max={refMp3.current !== Number ? 0 : refMp3.current?.duration}
