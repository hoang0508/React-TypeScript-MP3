...
export default function Playmusic() {
  ...
  const songs = dataPlayList?.playlist?.songs;
  console.log("🚀 ~ file: Playmusic.jsx ~ line 21 ~ Playmusic ~ songs", songs);

  // render khi hiển thị lần đầu nếu key là undefine
  const checkKey = songs ? songs[0]?.key : data?.song && data?.song[0].key;
  console.log(
    "🚀 ~ file: Playmusic.jsx ~ line 25 ~ Playmusic ~ checkKey",
    checkKey
  );
  ...
  console.log(
    "🚀 ~ file: Playmusic.jsx ~ line 32 ~ Playmusic ~ keySong",
    keySong
  );
  ...
  //data song length
  const dataSongLength = songs ? songs : data?.song;

  // data music key
  const MusicKeyData = songs
    ? songs &&
      songs.length > 0 &&
