...
export default function Playmusic() {
  ...
  function changeSong(dir) {
    if (dir === 1) {
      ...
      handleClickPlay();
      setTimeout(lodash.debounce(() => {}, 500));
    } else if (dir === -1) {
      setIndexSong(indexSong - 1);
      if (indexSong <= 0) {
  ...
  console.log(dataMusicKey?.streamUrls);
    ...
    <div className="flex justify-between flex-col gap-y-[80px]">
      ...
      <div>
        ...
        <div className="mt-4">
          ...
          {dataMusicKey && dataMusicKey?.streamUrls && indexSong > 1 && (
            <audio
              muted={false}
              src={dataMusicKey?.streamUrls[0]?.streamUrl}
              ref={refMp3}
            ></audio>
          )}
